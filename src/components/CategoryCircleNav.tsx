  ...
  return (
    <section className="bg-[#07241b] py-6 border-b border-[#2d5848] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4 sm:gap-6 overflow-x-auto px-5 py-2 scrollbar-none">
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`group flex flex-col items-center shrink-0 focus:outline-none ${cat.badge ? 'cursor-pointer': ''}`}
                disabled={!cat.badge}
              >
                {/* Circle Container */}
                <div
                  className={`w-18 h-18 sm:w-22 sm:h-22 rounded-full p-1 transition-all duration-300 relative ${isSelected
                ...
                {/* Name */}
                <span
                  className={`mt-2 text-xs sm:text-sm font-semibold transition-colors ${isSelected ? 'text-[#d4af37] font-bold' : 'text-gray-200 group-hover:text-[#d4af37]'
                    }`}
                >
                  {cat.name}
                </span>
